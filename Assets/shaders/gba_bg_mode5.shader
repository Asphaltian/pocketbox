...
{
	DevShader = true;
	Description = "GBA BG Mode 5 Bitmap 15-bit Small";
}
...
CS
{
	...
	void MainCs( uint3 id : SV_DispatchThreadID )
	{
		...
		if ( coord.x < 0 || coord.x >= ( 160 << 8 ) || coord.y < 0 || coord.y >= ( 128 << 8 ) )
		{
			OutputTex[id.xy] = float4( 0, 0, 0, 0 );
			return;
		}

		uint byteAddr = charBase + (uint)( ( coord.x >> 8 ) + ( coord.y >> 8 ) * 160 ) * 2u;
		uint color = LoadVramU16( Vram, byteAddr );
		OutputTex[id.xy] = Rgb555ToFloat4( color );
	}
}
